...
class SettingChangeCommand(Protocol):
    ...
    def execute(self, state: "AppSettingsState") -> None: ...
...
class BaseCommand(ABC):
    ...
    def undo(self) -> None:
        if self._state and self._setting_name and self._old_value:
            setattr(self._state, self._setting_name, self._old_value)

    def _execute(self, state, setting_name: str) -> None:
        if self._state is not None:
            return

        self._state = state
        self._setting_name = setting_name
        self._old_value = getattr(state, setting_name)
        setattr(state, setting_name, self._value)


class SetThemeCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._execute(state, "_theme")


class SetTyperBorderCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._execute(state.typer, "_border")


class SetTyperEngineCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._value = TyperEngine(self._value)
        self._execute(state.typer, "_engine")


class SetSingleLineEngineWidthCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._execute(state.typer.single_line_engine, "_width")


class SetSingleLineEngineStartFromCenterCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._execute(state.typer.single_line_engine, "_start_from_center")


class SetStandardEngineWidthCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._execute(state.typer.standard_engine, "_width")


class SetStandardEngineHeightCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._execute(state.typer.standard_engine, "_height")


class SetContentLanguageCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._value = Language(self._value)
        self._execute(state.content, "_language")


class SetContentTypeCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._value = ContentType(self._value)
        self._execute(state.content, "_content_type")


class SetContentLenghtCommand(BaseCommand):
    def execute(self, state: "AppSettingsState") -> None:
        self._execute(state.content, "_content_lenght")
